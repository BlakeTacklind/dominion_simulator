...
from fileWrites import fileWrites, closeFiles
...
class Player:
	...
	def check_for_card_type(self,lst,looking_for):
		number_found = 0
		for x in range(0,len(lst)):
			if lst[x].name == looking_for:
				number_found += 1
		return number_found


	def index_of_card_type(self,lst,looking_for):
		for x in range(0,len(lst)):
			if lst[x].name == looking_for:
				index = x
		return index
